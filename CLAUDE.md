...
doc consolidation. Two known gaps outside that: there is no
`docker-anvil.yml`, so `ghcr.io/safwyls/anvil` is never built, and the
GitHub repo is still named `sampo`.
