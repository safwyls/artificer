    ...
    <div className="max-h-96 space-y-2.5 overflow-y-auto pr-1">
      {entries.map(([key, value]) => (
        <div key={key} className="flex items-center justify-between gap-3">
          <span className="min-w-0 flex-1 break-all font-mono text-xs text-wk-parchment/70">{key}</span>
          <span className="shrink-0 whitespace-nowrap rounded-full bg-wk-parchment/5 px-2 py-1 font-mono text-xs text-wk-parchment/50">
            {formatValue(value)}
          </span>
        </div>
      ))}
