...
export function ServerPerformance({ serverId }: { serverId: number }) {
  ...
  return (
    <section className="rounded-2xl border border-wk-edge bg-wk-panel p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-base font-bold">Performance</h2>
          <p className="mt-0.5 text-xs text-wk-parchment/40">Sampled every 30s, kept for 7 days</p>
        </div>
        <div className="inline-flex rounded-lg border border-wk-edge bg-wk-parchment/5 p-0.5">
          {RANGES.map((r) => (
            <button
              key={r.minutes}
              ...
              className={cn(
                ...
                r.minutes === minutes ? "bg-wk-ember text-wk-parchment" : "text-wk-parchment/50 hover:text-wk-parchment",
              )}
            >
              {r.label}
            </button>
