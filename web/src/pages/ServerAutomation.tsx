    ...
    <div className="pb-24">
      <header className="sticky top-0 z-10 hidden items-center justify-between border-b border-ink/10 bg-paper px-8 py-6 lg:flex">
        <div>
          ...
          <p className="text-sm text-ink/60">Scheduled restarts and Discord notifications</p>
        </div>
      </header>
      ...
      {DAY_LETTERS.map((letter, i) => (
        <span
          ...
          className={cn(
            ...
            days.includes(i) ? "bg-ink text-paper" : "text-ink/25",
          )}
        >
          {letter}
        </span>
    ...
    <section className="rounded-xl border border-ink/10 bg-white lg:col-span-3">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-ink/5 px-5 py-4">
        <div className="flex items-center gap-2">
          <CalendarClock className="h-4 w-4 text-brand-amber" />
      ...
      {data.schedules.length === 0 ? (
        <div className="px-5 py-8 text-center">
          <p className="text-sm text-ink/60">No restart schedules yet.</p>
          {canEdit && (
            <p className="mt-1 text-sm text-ink/60">
              Add one and Palcon warns players in-game, saves the world, then restarts the server.
            </p>
          )}
        </div>
      ) : (
      ...
      <div className="space-y-1 border-t border-ink/5 px-5 py-3 text-xs text-ink/50">
        <p>
          Times are {data.timezone} (Palcon's clock). The world is saved before every restart.
        </p>
        {!data.dockerRestart && (
                ...
                <button
                  ...
                  className={cn(
                    ...
                    days.includes(i)
                      ...
                      : "border border-ink/15 text-ink/40 hover:border-ink/30 hover:text-ink",
                  )}
                  onClick={() => toggleDay(i)}
                >
                  {letter}
...
function WatchdogCard({ serverId, config }: { serverId: number; config: { enabled: boolean; available: boolean } }) {
  ...
  return (
    <section className="rounded-xl border border-ink/10 bg-white lg:col-span-3">
      ...
      <div className="space-y-2 px-5 py-4 text-sm text-ink/60">
        <p>
        ...
        <p className="text-xs text-ink/45">
          Clean stops are left alone — stopping through Palcon always reads as one. Stopping the container behind
          Palcon's back (TrueNAS UI, docker stop) can end in a force-kill that looks like a crash and will be revived;
  ...
  return (
    <section className="rounded-xl border border-ink/10 bg-white lg:col-span-2">
      <div className="flex items-center justify-between border-b border-ink/5 px-5 py-4">
        <div className="flex items-center gap-2">
          <Webhook className="h-4 w-4 text-pal-blue" />
      ...
      <div className="space-y-4 px-5 py-4">
        {config.configured ? (
          <div className="flex items-center gap-2 text-sm text-ink/70">
            <span className="h-2 w-2 rounded-full bg-pal-blue" />
            Webhook connected
          </div>
        ) : (
          <p className="text-sm text-ink/60">
        ...
        {config.configured && (
          <>
              ...
              {events.map((ev) => (
                <li key={ev.key} className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-ink/50">{ev.help}</p>
                  </div>
                  <Switch
            ...
            <div className="flex items-center justify-between border-t border-ink/5 pt-3">
              ...
              <button
                className="text-xs text-ink/40 underline-offset-2 hover:text-destructive hover:underline"
                disabled={remove.isPending}
