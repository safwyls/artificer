...
const segmentClass = ({ isActive }: { isActive: boolean }) =>
  cn(
    ...
    isActive ? "bg-ft-spore text-ft-bone" : "text-ft-bone/60",
  );
...
export function MobileTopBar({ server }: { server: Server | null }) {
  ...
  const menuItem =
    "flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-sm text-ft-bone hover:bg-ft-bone/5 transition-colors";

  return (
    <div className="shrink-0 bg-ft-void px-4 pb-3 pt-[max(1rem,env(safe-area-inset-top))] text-ft-bone">
      <div className="flex items-center justify-between">
        {server ? (
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            >
              <span className="flex h-full w-full items-center justify-center rounded-full bg-ft-fog font-display text-xs font-bold">
                {initials(server.name)}
              </span>
            </span>
            <div className="min-w-0">
              <p className="truncate font-display text-sm font-bold leading-tight">{server.name}</p>
              <p className="font-mono text-[11px] text-ft-bone/50">
                {infoQuery.isSuccess
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-full border border-ft-stone bg-ft-panel font-ftdisplay text-base text-ft-flame">F</div>
            <p className="font-display text-sm font-bold">Flametender</p>
          </div>
        )}
        ...
        <div className="relative">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-full bg-ft-panel text-sm text-ft-bone/70"
          >
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 top-10 z-30 w-52 overflow-hidden rounded-xl border border-ft-edge bg-ft-void text-ft-bone shadow-lg">
                {server && (
                  <>
                    ...
                    >
                      <RefreshCw className="h-4 w-4 text-ft-bone/50" /> Refresh
                    </button>
                    ...
                    >
                      <Gamepad2 className="h-4 w-4 text-ft-bone/50" /> Copy join address
                    </button>
                    {can("save") && (
                      <button
                        className={menuItem}
                      ...
                      >
                        <Save className="h-4 w-4 text-ft-bone/50" /> Save world
                      </button>
                    )}
                    {can("shutdown") && (
                      <button
                      ...
                      >
                        <Power className="h-4 w-4 text-ft-spore" /> Shut down…
                      </button>
                    )}
                    {isAdmin && (
                      <>
                        ...
                        >
                          <Pencil className="h-4 w-4 text-ft-bone/50" /> Edit server…
                        </button>
                        <button
                          className={menuItem}
                          onClick={() => {
                        ...
                        >
                          <Trash2 className="h-4 w-4 text-ft-bone/50" /> Remove server…
                        </button>
                      </>
                    )}
                ...
                >
                  <LogOut className="h-4 w-4 text-ft-bone/50" /> Log out {username}
                </button>
              </div>
            </>
          )}
      ...
      {server && (
        <div ref={navRef} className="no-scrollbar scroll-fade-x mt-3 flex gap-0.5 overflow-x-auto rounded-xl bg-ft-panel p-1">
...
export function MobileBottomRail({ servers, activeServerId }: { servers: Server[]; activeServerId: number | null }) {
  ...
  return (
    <div className="flex shrink-0 items-center justify-around border-t border-black/20 bg-ft-void pt-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))]">
      {servers.map((server) => (
        <FtServerFlame
          key={server.id}
          server={server}
      ...
      {isAdmin && (
        <>
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-dashed border-white/20 text-ft-bone/40"
          >
