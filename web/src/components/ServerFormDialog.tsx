          ...
          {mode === "create" && kind === "supervised" && candidates.length > 0 && (
            <div className="mt-3 space-y-1.5">
              ...
              {candidates.map((c) => (
                ...
                >
                  <span className={cn("h-2 w-2 shrink-0 rounded-full", c.running ? "bg-pal-green" : "bg-ink/30")} />
          ...
          <div id={`server-form-${kind}`} role="tabpanel" aria-label={`${kind} settings`}>
            ...
            {kind === "supervised" ? (
              <Section title="Agent">
                ...
                {strays.length > 0 && (
                  <div className="rounded-xl border border-brand-amber/40 bg-brand-amber/10 px-3 py-2 text-xs text-ink/60">
                    <p>
                    ...
                    <button
                      ...
                      className="mt-1.5 font-semibold text-brand-red hover:underline"
                    >
              ...
              <>
                ...
                <Section title="File access">
                  ...
                  <details
                    ...
                    className="rounded-xl border border-ink/10 px-3 pb-3"
                  >
...
function ModeTabs({ kind, onChange }: { kind: Kind; onChange: (k: Kind) => void }) {
  return (
    <div role="tablist" aria-label="Deployment mode" className="grid grid-cols-2 gap-1 rounded-xl border border-ink/10 bg-ink/5 p-1">
...
function WiringReadout({ caps, hint }: { caps: Cap[]; hint: string }) {
  return (
    <div className="mt-3 rounded-xl border border-ink/10 bg-ink/5 px-3 py-2">
      <div className="flex flex-wrap gap-x-4 gap-y-1 font-mono text-xs">
        {caps.map((c) => (
          <span key={c.label} className={c.on ? "text-pal-green" : "text-ink/35"}>
