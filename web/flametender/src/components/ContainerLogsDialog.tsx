    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[85vh] w-[calc(100vw-2rem)] max-w-4xl flex-col">
            ...
            <span className="flex items-center gap-1.5 font-mono text-xs font-normal text-ft-bone/45">
              ...
              <span
                className={cn(
                  ...
                  paused ? "bg-ft-bone/25" : "animate-pulse bg-ft-ok",
                )}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            className="flex items-center gap-1.5 rounded-lg border border-ft-edge px-2.5 py-1.5 text-xs font-semibold text-ft-bone/60 hover:bg-ft-bone/5"
          ...
          <label className="ml-auto flex cursor-pointer items-center gap-1.5 text-xs text-ft-bone/60">
            <input
              type="checkbox"
              checked={hideRest}
              onChange={(e) => setHideRest(e.target.checked)}
              className="h-3.5 w-3.5 accent-ft-spore"
            />
            Hide REST polling
            {hideRest && hiddenCount > 0 && <span className="font-mono text-ft-bone/35">· {hiddenCount} hidden</span>}
          </label>
        </div>
