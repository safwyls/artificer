...
export function RaiseServerDialog({
  ...
  return (
    ...
    >
      <DialogContent className="flametender max-h-[90vh] overflow-y-auto border-ft-edge bg-ft-panel font-ftbody text-ft-bone sm:max-w-xl">
        {result ? (
          ...
          <>
            ...
            <div className="ft-toplight rounded-md border border-ft-edge bg-gradient-to-br from-ft-fog to-[#141b16] px-5 py-4">
              <Label htmlFor="raise-join-password" className="text-sm font-bold tracking-[0.06em] text-ft-stonehi">
                Join password
              </Label>
              <Input
                ...
                spellCheck={false}
                className="mt-2 border-ft-edge bg-ft-void font-mono text-sm text-ft-bone"
              />
              <p className="mt-2 text-xs text-ft-lichen">
                Leave blank for an <span className="text-ft-spore">open server</span> — anyone who finds it in the
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <LabelledField
                ...
                hint="Where Flametender and players reach it"
              />
              <div className="space-y-1.5">
                <Label className="text-ft-lichen">Game port</Label>
                <NumberField value={gamePort} onChange={setGamePort} min={1} max={65535} />
                <p className="text-xs text-ft-lichen">
                  One UDP port — <span className="font-mono text-ft-bone/70">{gamePort}</span> carries the game and the
                  Steam query both.
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs uppercase tracking-[0.1em] text-ft-lichen transition hover:text-ft-bone"
            >
              <ChevronDown className={cn("h-3.5 w-3.5 transition", advanced && "rotate-180")} />
              Deployment details
            </button>
            {advanced && (
              <div className="grid grid-cols-2 gap-3 rounded-md border border-ft-edge bg-ft-void/40 p-3">
                <div className="space-y-1.5">
                  <Label className="text-ft-lichen">Agent port</Label>
...
function RaiseResult({ result, onClose }: { result: ProvisionResult; onClose: () => void }) {
  return (
    <>
      ...
      {result.deployError && (
        <p className="rounded border border-ft-sporedim bg-ft-spore/5 px-3 py-2 text-sm text-ft-spore">
      ...
      <p className="-mt-1 text-xs text-ft-lichen">
      ...
      {result.dataDir && (
        <p className="text-xs text-ft-lichen">
          World data lives at <span className="font-mono text-ft-bone/70">{result.dataDir}</span> and survives the
          container.
        </p>
      )}
      ...
      <div className="flex justify-end">
        <button
          onClick={onClose}
          className="rounded border border-ft-stone bg-gradient-to-b from-[#2b2f26] to-[#1d211a] px-4 py-1.5 text-sm font-bold tracking-[0.05em] text-ft-stonehi transition hover:brightness-125"
        >
          Done
        </button>
      </div>
...
function Secret({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div>
      <Label className="text-ft-lichen">{label}</Label>
      <div className="mt-1.5 flex items-center gap-2 rounded bg-ft-void px-3 py-2.5">
        <code className="flex-1 break-all font-mono text-sm text-ft-bone">{value}</code>
        <button
          onClick={async () => {
          ...
          className="shrink-0 text-ft-lichen transition hover:text-ft-bone"
        >
    ...
    <div>
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-ft-stonehi transition hover:text-ft-bone"
        >
          {copied ? <Check className="h-3.5 w-3.5 text-ft-ok" /> : <Copy className="h-3.5 w-3.5" />}
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
      <pre className="mt-1.5 max-h-64 overflow-auto rounded bg-ft-void px-3 py-2.5 font-mono text-xs leading-relaxed text-ft-bone/80">
        {value}
      </pre>
    </div>
  );
  ...
  return (
    <div className="space-y-1.5">
      ...
      <Input
        ...
        className="border-ft-edge bg-ft-void text-ft-bone"
      />
      {hint && <p className="text-xs text-ft-lichen">{hint}</p>}
    </div>
  );
}
