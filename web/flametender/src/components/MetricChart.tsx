...
export function MetricChart({
  ...
  return (
    <div className={cn("rounded-xl border border-ft-edge bg-ft-panel p-4", className)}>
      <div className="flex items-baseline justify-between gap-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-ft-bone/50">{label}</p>
        {hasData && activeValue !== null && (
          <p className="font-mono text-lg font-bold" style={{ color }}>
            {format(activeValue)}
            {unit && <span className="ml-0.5 text-xs font-medium text-ft-bone/40">{unit}</span>}
          </p>
        )}
      </div>

      {!hasData ? (
        <p className="flex h-[90px] items-center justify-center text-xs text-ft-bone/40">
        ...
        <>
          ...
          <div className="mt-1 flex justify-between font-mono text-[10px] text-ft-bone/35">
            <span>
