...
function KindBadge({ c }: { c: HostContainer }) {
  switch (containerKind(c)) {
    case "registered":
      return (
        <Link
          ...
          className="text-xs font-semibold text-ft-flame hover:underline"
        >
          {c.serverName}
        </Link>
      );
    case "orphan":
      ...
      return (
        <span className="inline-flex items-center gap-1 rounded-full bg-ft-stonehi/15 px-2 py-0.5 text-xs font-semibold text-ft-stonehi">
          <AlertTriangle className="h-3 w-3" />
          not registered — adoptable
        </span>
      );
    case "foreign":
      return <span className="text-xs text-ft-bone/50">{c.owner}</span>;
    case "unmanaged":
      return <span className="text-xs text-ft-bone/40">not managed by Anvil</span>;
  }
}

function SectionError({ message }: { message: string }) {
  return (
    <div className="flex items-start gap-2 rounded-xl border border-ft-spore/30 bg-ft-spore/10 p-3 text-sm text-ft-spore">
...
function SummaryTile({ label, value, detail }: { label: string; value: string; detail?: string }) {
  return (
    <div className="rounded-2xl border border-ft-edge bg-ft-panel p-4">
      <p className="text-xs font-semibold uppercase tracking-wide text-ft-bone/45">{label}</p>
      <p className="mt-1 text-xl font-bold text-ft-bone">{value}</p>
      {detail && <p className="mt-0.5 truncate text-xs text-ft-bone/50">{detail}</p>}
    </div>
  );
}
      ...
      {rows.length > 0 && (
        <div className="overflow-x-auto rounded-2xl border border-ft-edge bg-ft-panel">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((c) => (
                <tr key={c.name} className="border-b border-ft-edge/50 last:border-0">
                  <td className="px-4 py-2.5">
                    <span className="font-mono text-xs font-semibold text-ft-bone">{c.name}</span>
                    {c.dataDir && <p className="font-mono text-[11px] text-ft-bone/40">{c.dataDir}</p>}
                  </td>
                  <td className="px-4 py-2.5">
                    <StateBadge c={c} />
                    {c.status && <p className="mt-0.5 text-[11px] text-ft-bone/45">{c.status}</p>}
                  </td>
                  <td className="px-4 py-2.5">
                    <KindBadge c={c} />
                  </td>
                  <td className="px-4 py-2.5 font-mono text-xs text-ft-bone/60">{c.image}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-ft-bone/60">{portsLabel(c) || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {rows.length > 0 && (
        <div className="overflow-x-auto rounded-2xl border border-ft-edge bg-ft-panel">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((img) => (
                <tr key={img.id} className="border-b border-ft-edge/50 last:border-0">
                  <td className="px-4 py-2.5 font-mono text-xs text-ft-bone">
                    {imageName(img)}
                    {img.tags.length === 0 && (
                  ...
                  <td className="px-4 py-2.5 text-xs text-ft-bone/70">{formatBytes(img.size)}</td>
                  <td className="px-4 py-2.5 font-mono text-xs text-ft-bone/60">
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="space-y-8 p-4 lg:p-8">
        {overviewQuery.isLoading && <p className="text-sm text-ft-bone/50">Asking Anvil…</p>}
        {overviewQuery.isError && (
          <SectionError message={errorDetail(overviewQuery.error) ?? "Failed to load the host overview"} />
        )}

        {overview && !overview.available && (
          <div className="rounded-2xl border border-ft-edge bg-ft-panel p-6">
            <p className="font-semibold text-ft-bone">No host service connected</p>
            <p className="mt-1 max-w-prose text-sm text-ft-bone/60">{overview.reason}</p>
          </div>
        )}
