...
const OVERVIEW: HostOverview = {
  ...
  health: {
    ...
    version: "1.4.0",
    client: "flametender",
    dataRoot: "/mnt/tank/es",
    dockerOk: true,
  },
  containers: [
    {
      name: "flameagent-grimwood",
  ...
  ports: [{ port: 8211, proto: "udp", container: "flameagent-grimwood" }],
  images: [
    {
      id: "sha256:fl1",
      tags: ["ghcr.io/safwyls/flameagent:latest"],
      size: 900_000_000,
      created: 1_755_000_000,
      containers: ["flameagent-grimwood", "flameagent-lost"],
    },
...
describe("Host", () => {
  ...
  it("shows every container on the host and links registered ones to their server", async () => {
    ...
    const link = await screen.findByRole("link", { name: "Grimwood" });
  ...
  it("shows images with their disk cost and flags the dangling one", async () => {
    ...
    expect(screen.getByText("858 MB")).toBeInTheDocument(); // sha256:fl1
  ...
  it("keeps the sections that worked when one read fails", async () => {
    ...
    await screen.findByRole("link", { name: "Grimwood" });
