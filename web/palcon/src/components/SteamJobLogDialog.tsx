      ...
      <DialogContent className="flex max-h-[85vh] w-[calc(100vw-2rem)] max-w-4xl flex-col">
        <DialogHeader>
          <DialogTitle className="flex flex-wrap items-center gap-x-3 gap-y-1">
            SteamCMD update
            {job && (
              <span className="flex items-center gap-1.5 font-mono text-xs font-normal text-ink/45">
                {job.state === "running"
                ...
                <span
                  className={cn(
                    ...
                    running ? "animate-pulse bg-pal-green" : job.state === "done" ? "bg-pal-green" : "bg-brand-red",
                  )}
        ...
        {job?.error && (
          <p className="rounded-lg bg-brand-red/10 px-3 py-2 font-mono text-xs text-brand-red">{job.error}</p>
        )}

        <div className="flex items-center gap-2">
          <button
            className="flex items-center gap-1.5 rounded-lg border border-ink/15 px-2.5 py-1.5 text-xs font-semibold text-ink/60 hover:bg-ink/5 disabled:opacity-50"
          ...
          <span className="text-xs text-ink/40">Last {lines.length} lines from the agent</span>
        </div>
        ...
        <pre
          ...
          className="min-h-48 flex-1 overflow-auto whitespace-pre-wrap break-all rounded-xl bg-ink p-4 font-mono text-[11px] leading-relaxed text-paper/80"
        >
          {lines.length > 0
            ? lines.join("\n")
            : job
