...
export function PublicStatus() {
  ...
  return (
    <div className="app-min-height flex flex-col items-center justify-center bg-paper p-6">
      <main className="clip-notch-lg w-full max-w-md border border-ink/10 bg-white px-8 py-10 text-center shadow-sm">
        {unknown ? (
          <>
          ...
          <p className="text-sm text-ink/60">Status is unavailable right now. Refresh to try again.</p>
        ) : status ? (
          <>
            ...
            <p
              className={cn(
                ...
                status.online ? "bg-pal-green/15 text-pal-green" : "bg-ink/10 text-ink/50",
              )}
            >
              <span className={cn("h-2.5 w-2.5 rounded-full", status.online ? "bg-pal-green" : "bg-ink/30")} />
              ...
              <div className="mt-6">
                <p className="font-mono text-5xl font-semibold tabular-nums">
                  ...
                  {status.maxPlayers !== undefined && (
                    <span className="text-2xl text-ink/35">/{status.maxPlayers}</span>
                  )}
                </p>
                <p className="mt-1 text-xs uppercase tracking-widest text-ink/40">players online</p>
              </div>
            )}

            {countdown && (
              <p className="mt-6 text-sm text-ink/60">
                Next scheduled restart <span className="font-mono font-semibold text-ink">{countdown}</span>
              </p>
            )}
          </>
        ) : null}
      </main>
      <p className="mt-4 font-mono text-[11px] text-ink/35">powered by Palcon</p>
    </div>
  );
}
