...
// choose its own. Consoles override via Server.SessionCookie so existing
// sessions survive their port to the monorepo.
const defaultSessionCookie = "console_session"
...
// cookieName is the session cookie's name — per console, so two consoles
// on one origin don't clobber each other's sessions.
