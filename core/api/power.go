...
func (s *Server) handleContainerAction(w http.ResponseWriter, r *http.Request) {
	...
	if agent, _ := s.agentSupervisor(ctx, srv); agent != nil {
		...
		switch {
		case action == "restart" && s.bans != nil && s.bans.Pending(ctx, srv):
			if _, err = agent.Power(ctx, "stop", graceful); err == nil {
				s.applyOfflineWork(ctx, srv)
...
// applyOfflineWork runs the game's queued offline config edits, if any.
func (s *Server) applyOfflineWork(ctx context.Context, srv *store.Server) {
	if s.bans != nil {
