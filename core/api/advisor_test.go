...
func TestDocsEndpoint(t *testing.T) {
	...
	// The fixture FS stands in for a console's embedded docs; which files
	// exist is the console's business, so the assertion is only that the
	// endpoint serves what the FS holds, content included.
	found := false
	for _, d := range docs {
		m := d.(map[string]any)
		if m["name"] == "advisor.md" && len(m["content"].(string)) > 0 {
			found = true
		}
	}
	if !found {
		t.Error("advisor.md missing from docs payload")
	}
}
