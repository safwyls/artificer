...
func TestGeneratedStackIsValidYAML(t *testing.T) {
	...
	svc, ok := parsed.Services["gtagent"]
	if !ok {
		t.Fatalf("no gtagent service in:\n%s", res.Stack)
	}
	if svc.Env["GTAGENT_SERVER_NAME"] != `Quote"Name` {
		t.Errorf("server name did not survive quoting: %q", svc.Env["GTAGENT_SERVER_NAME"])
	}
	if svc.Env["GTAGENT_JOIN_PASSWORD"] != `pass"word` {
		t.Errorf("join password did not survive quoting: %q", svc.Env["GTAGENT_JOIN_PASSWORD"])
	}
	...
	if len(svc.Volumes) != 1 || svc.Volumes[0] != "/mnt/pool/es:/gametest" {
