...
// fake agent fixtures below. Game modules assert their real values in
// their own port phases.
var testProfile = &api.ProvisionProfile{
	AgentName:       "gtagent",
	...
	StackHeadline:   "REST test game server supervised by gtagent",
	StackNotes:      "# On first boot the agent installs the game via SteamCMD.\n",
	GamePortComment: "game port (single UDP)",
}
...
type fakeProvisioner struct {
	...
	healthErr    error
	provisionReq *agent.ProvisionRequest
...
func (f *fakeProvisioner) RecreateAgent(ctx context.Context, container, imageTag string) (*agent.RecreateResult, error) {
...
func TestProvisionServer(t *testing.T) {
	...
	for _, want := range []string{
		"ghcr.io/example/gtagent:beta",
		`user: "568:568"`,
		"HOME: /tmp",
		"GTAGENT_MODE: supervisor",
		"GTAGENT_TOKEN: " + res.AgentToken,
		"GTAGENT_ADMIN_PASSWORD: " + res.AdminPassword,
		`GTAGENT_SERVER_NAME: "Grimwood II"`,
		`GTAGENT_JOIN_PASSWORD: "sesame-open"`,
		`"25637:25600/udp"`, `"9811:8811"`,
		"/mnt/pool/apps/es-g2:/gametest",
	} {
	...
	for _, reject := range []string{"25638", "GTAGENT_OWNER_ID", "GTAGENT_WORLD_NAME"} {
...
func TestProvisionOneClickDeploy(t *testing.T) {
	...
	fake := &fakeProvisioner{
		...
		provisionRes: &agentctl.ProvisionResult{
			Container: "gtagent-one-click", DataDir: filepath.Join(dataRoot, "one-click"),
		},
	}
	...
	if !strings.Contains(res.Stack, filepath.Join(dataRoot, "one-click")+":/gametest") {
		t.Errorf("stack volume line missing the resolved data dir:\n%s", res.Stack)
	}
	if res.Server.GamePort != testProfile.DefaultGamePort {
		t.Errorf("gamePort = %d, want the default %d", res.Server.GamePort, testProfile.DefaultGamePort)
	}
	...
	if res.Server.ContainerName != "gtagent-one-click" {
		t.Errorf("containerName = %q, want gtagent-one-click", res.Server.ContainerName)
	}
}
...
func TestDeleteServerDestroysContainerWhenAsked(t *testing.T) {
	...
	fake := &fakeProvisioner{
		destroyRes: &agentctl.DestroyResult{
			Container: "gtagent-doomed", DataDir: filepath.Join(dataRoot, "doomed"),
		},
	}
	app.api.Provisioner = fake
	...
	if res.Destroyed != "gtagent-doomed" || res.DataDir != filepath.Join(dataRoot, "doomed") {
...
func TestDeleteServerKeepsRowWhenDestroyRefused(t *testing.T) {
	...
	rec := app.do(t, "POST", "/api/servers", map[string]any{
		"name": "By Hand", "host": "10.0.0.9", "enabled": true, "containerName": "gtagent-byhand",
	}, admin)
	if rec.Code != http.StatusCreated {
		t.Fatalf("create server: %d (body %s)", rec.Code, rec.Body)
	}
...
func TestProvisionNameConflictRegistersNothing(t *testing.T) {
	...
	fake := &fakeProvisioner{
		discovered: []agentctl.DiscoveredServer{
			{Name: "gtagent-taken", Image: "ghcr.io/example/gtagent:latest", Running: true, AgentPort: 8811},
		},
	}
	app.api.Provisioner = fake
	...
	if !strings.Contains(rec.Body.String(), "gtagent-taken") {
...
func TestProvisionDefaultsAndDiscover(t *testing.T) {
	...
	fake := &fakeProvisioner{
		...
		discovered: []agentctl.DiscoveredServer{
			{Name: "gtagent-adopted", Image: "ghcr.io/example/gtagent:beta", Running: true, GamePort: 25637, AgentPort: 9811},
			{Name: "gtagent-orphan", Image: "ghcr.io/example/gtagent:beta", Running: false, GamePort: 25638, AgentPort: 9911},
		},
		adoptRes: &agentctl.AdoptResult{
			Name: "gtagent-orphan", Mode: "supervisor", ServerName: "Orphaned World",
	...
	if _, err := app.store.CreateServer(t.Context(), &store.Server{
		Name: "existing", Host: "10.99.0.5", GamePort: testProfile.DefaultGamePort,
	...
	used := map[int]bool{testProfile.DefaultGamePort: true, 25637: true, 25638: true, 9811: true, 9911: true}
	...
	if !byName["gtagent-adopted"] || byName["gtagent-orphan"] {
	...
	rec = app.do(t, "POST", "/api/servers/adopt", map[string]string{"container": "gtagent-orphan"}, admin)
	if rec.Code != http.StatusCreated {
