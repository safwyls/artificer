...
func (s *Server) handleDocs(w http.ResponseWriter, r *http.Request) {
	if s.DocsFS == nil {
		writeJSON(w, http.StatusOK, map[string]any{"docs": []any{}})
		return
	}
	entries, err := fs.Glob(s.DocsFS, "*.md")
	if err != nil {
	...
	for _, name := range entries {
		content, err := fs.ReadFile(s.DocsFS, name)
		if err != nil {
			writeError(w, http.StatusInternalServerError, "failed to read docs")
			return
		}
...
func (s *Server) LoadStoredAdvisor(ctx context.Context) (string, error) {
	...
	client, err := advisor.New(ctx, key.Provider, key.APIKey, key.Model, s.AdvisorPrompt)
	if err != nil {
		return "", err
	}
	s.advisorMu.Lock()
...
func (s *Server) advisorFor(r *http.Request) (AdvisorClient, string, error) {
	if user, ok := userFromContext(r.Context()); ok {
		...
		if err != nil {
			...
			client, err := advisor.New(context.Background(), key.Provider, key.APIKey, key.Model, s.AdvisorPrompt)
			if err != nil {
...
func (s *Server) handleSetUserAdvisorKey(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := advisor.New(context.Background(), req.Provider, req.APIKey, req.Model, s.AdvisorPrompt); err != nil {
...
func (s *Server) handleSetAdvisorKey(w http.ResponseWriter, r *http.Request) {
	...
	client, err := advisor.New(context.Background(), req.Provider, req.APIKey, req.Model, s.AdvisorPrompt)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (s *Server) handleSetAdvisorKeyModel(w http.ResponseWriter, r *http.Request) {
	...
	key.Model = req.Model
	client, err := advisor.New(context.Background(), key.Provider, key.APIKey, req.Model, s.AdvisorPrompt)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
