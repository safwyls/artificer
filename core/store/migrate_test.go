...
func TestUpgradeBackfillsGameOnExistingRows(t *testing.T) {
	...
	// The column backfills empty — "which game" is the console's call, so
	// the invariant is that the row still resolves to the default game,
	// not that the schema named one.
	if def, ok := srv.Definition(); !ok || def.ID != game.DefaultID {
		t.Errorf("upgraded row resolves to %v, want the default game %q", def, game.DefaultID)
	}
