...
func newSupervisor(cfg Config, jobsBusy func() bool) *supervisor {
	port := cfg.GamePort
	if port <= 0 {
		port = cfg.Game.DefaultGamePort
	}
	grace := cfg.StopGrace
	if grace <= 0 {
		// The game says how long a graceful stop deserves: killing a
		// mid-save server is how worlds corrupt, so games that save on
		// the way down set this generously.
		grace = cfg.Game.stopGrace()
	}
...
func (s *supervisor) desiredPath() string {
	// The dot-dir keeps the agent's brand name so a console's existing
	// deployments keep their persisted desired state across the port to
	// the kit.
	return filepath.Join(s.installDir, "."+s.spec.AgentName, "desired")
}
...
func (s *supervisor) Stop(selfExit time.Duration) error {
	...
	signalGroup(pid, s.spec.stopSignal(), false)
	select {
	case <-done:
	...
	s.logger.Warn("game ignored the stop signal; killing", "signal", s.spec.stopSignal(), "grace", s.grace)
	signalGroup(pid, s.spec.stopSignal(), true)
	select {
	case <-done:
		return nil
...
// prepareRuntime runs the game's pre-start hook, which makes a freshly
// installed server bootable and keeps the operator's identity settings
// authoritative — what that concretely means (seeding a config, enforcing
// role passwords) is the game's knowledge, not the kit's.
