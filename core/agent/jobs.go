...
type jobRunner struct {
	...
	// nothing is running — what /v1/health reports so flametender can
...
// log as plain text, same as flametender does for container logs.
...
func (r *jobRunner) run(id, command string, args []string) {
	...
	for attemptNo := 1; ; attemptNo++ {
		...
		r.appendLog(id, fmt.Sprintf(
			"agent: retrying (%d/%d) — %q is SteamCMD's usual cold-bootstrap flake and normally clears on the second run",
