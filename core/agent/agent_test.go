...
// testGame is the kit's fake game: JSON config, savegame dir, and the
// custom-command launch path (the real launch assembly is each game
// module's). ValidateConfig is a real JSON check so the refuse-bad-
// uploads path stays exercised.
func testGame() agent.Game {
	return agent.Game{
		AgentName:         "gtagent",
		AppID:             4000001,
		DefaultGamePort:   25600,
		ConfigRelPath:     "gametest.json",
		...
		ValidateConfig: func(data []byte) error {
			...
			if doc.QueryPort > 65535 {
				return errors.New("queryPort out of range")
			}
			return nil
		},
	}
...
func newTestAgent(t *testing.T, script string) (*httptest.Server, string) {
	...
	if err := os.WriteFile(filepath.Join(install, "steamapps", "appmanifest_4000001.acf"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestAgentRejectsShortToken(t *testing.T) {
	_, err := agent.New(agent.Config{Token: "short", InstallDir: t.TempDir()})
	if err == nil {
...
func TestAgentRejectsRetiredProvisionerMode(t *testing.T) {
	_, err := agent.New(agent.Config{
		Token: testToken, InstallDir: t.TempDir(), Mode: "provisioner", Game: testGame(),
	})
...
func TestAgentAuth(t *testing.T) {
	...
	if health["apiVersion"] != float64(agent.APIVersion) || health["mode"] != "companion" || health["installDirOk"] != true {
...
echo "Success! App '2278520' fully installed."`)
	...
	if !strings.Contains(joined, "agent: retrying (2/2)") {
