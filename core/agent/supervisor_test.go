...
func newSupervisorAgent(t *testing.T, gameScript string) (*httptest.Server, *agent.Agent, string) {
	t.Helper()
	install := t.TempDir()
	...
	agent, err := agent.New(agent.Config{
		Token: testToken, InstallDir: install, SteamCmd: steamcmd, Version: "test", Game: testGame(),
		Mode:                "supervisor",
		GameCommand:         "./game.sh",
...
func TestSupervisorLifecycle(t *testing.T) {
	...
	desired, err := os.ReadFile(filepath.Join(install, ".gtagent", "desired"))
...
// The kit's half of config seeding/enforcement is the hook contract:
// PrepareRuntime runs before every start, with the profile's config path
// and exactly the identity the operator configured. What the hook then
// does to the file (seed a full config, enforce role passwords, keep a
// .bak) is the game module's behavior, asserted in the game's own tests
// — flametender's enforcement suite returns with games/enshrouded at its
// port (drift ledger, agent-kit row).
func TestPrepareRuntimeHookRunsBeforeEveryStart(t *testing.T) {
	install := t.TempDir()
	...
	a, err := agent.New(agent.Config{
		Token: testToken, InstallDir: install, SteamCmd: "/bin/true", Version: "test", Game: game,
	...
	defer mu.Unlock()
	if len(calls) != 1 {
		t.Fatalf("PrepareRuntime ran %d times for one start, want 1", len(calls))
	}
	want := agent.RuntimeIdentity{
		ServerName: "Grimwood Bastion", GamePort: 25637,
		AdminPassword: "hunter2-but-longer", JoinPassword: "friends-only",
	}
	if calls[0] != want {
		t.Errorf("PrepareRuntime identity = %+v, want %+v", calls[0], want)
	}
	if paths[0] != filepath.Join(install, "gametest.json") {
		t.Errorf("PrepareRuntime path = %q, want the profile's config path", paths[0])
	}
}
