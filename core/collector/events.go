...
func (c *Collector) watch(ctx context.Context, srv *store.Server, client game.Client) {
	...
	// event: it's the record of how much flametender actually watched.
...
// resume reconciles the events table on flametender's first look at a server.
//
// Flametender's own downtime is invisible from the server's side: a player who
...
// the last instant flametender was actually watching, then open a fresh session
// for whoever is on the server now. Time flametender did not observe belongs to
...
func (c *Collector) resume(ctx context.Context, srv *store.Server, current map[string]watched, lastSeen, now time.Time) {
	...
	// Deliberately silent on Discord: flametender starting up is not player
...
// closeSessions ends every session flametender currently believes is open. Run
