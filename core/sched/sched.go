...
// keeps a rebooted Flametender from replaying events that are more than a
...
type Scheduler struct {
	...
	logger *slog.Logger
	// bans is the game's queued offline config work (ban edits and the
	// like), applied during the restart between the stop and the start —
	// the only moment the game isn't holding its config file. Nil when
	// the game has none.
...
// the game is down: recorded while it runs, applied between a stop and
// the next start. The game module supplies the implementation (Enshrouded's
// ban queue is the motivating case); a game without the problem passes nil.
...
func New(st *store.Store, notifier *notify.Notifier, docker *dockerctl.Client, logger *slog.Logger, bans OfflineConfigWork) *Scheduler {
...
func (s *Scheduler) restart(ctx context.Context, srv *store.Server, sc *store.RestartSchedule) {
	...
	// looking at the same machine flametender's docker proxy is.
	...
	case agent != nil:
		...
		if s.bans != nil && s.bans.Pending(ctx, srv) {
