...
// already serve (the `/v1/api/*` paths inherited from flametender's Palworld
...
func init() {
	...
	// The shared tests predate multi-game rows; letting the fake be the
	// default keeps them writing rows without naming a game. A console
	// binary sets this for real at startup — first registration wins here
	// only because nothing else is registered in a shared-layer test.
	if game.DefaultID == "" {
		game.DefaultID = ID
	}
}
