...
| HEAD | `/v1/files/save` | companion, supervisor | the bundle ETag alone — the restore precondition's input (empty install answers the empty set's ETag). API version 4; an agent below it answers **405**, not 404, because GET on this path predates it — the console reads that as "update the agent image" |
