...
agent container, replacing the pile of bind mounts the base needs otherwise.
(Written for palcon, where the pattern originated; the mechanism is
game-agnostic and the specifics below are Dragonwilds'.)

## Why

Everything the dashboard does beyond the game's own admin surface rests on
one assumption: the control plane
...
mount wired into the control plane's container, and none of them work when the game
server lives on a different machine. The recurring failure that motivated
this design: a game update corrupts the game container's SteamCMD
...
the base becomes a pure control plane speaking HTTP to it.
...
running the server; the agent mounts the same `/dragonwilds` volume and absorbs
the file-side features:

- SteamCMD repair: clear `steamapps/*` + `steam/packages/*`, and run
  `steamcmd +app_update 4019830 validate` itself against the shared volume
...
- File verbs (phase 2): the world save directory as a tar bundle
  ...
  `DedicatedServer.ini` GET/PUT (atomic write). The base mirrors these into
...
cannot see the game process (separate container), so the base — which can —
...
(`/v1/power/*`); the base routes power, status and logs to the agent
...
level down) — which also means the base's existing in-game-shutdown
...
Supervisor-mode stack (replaces the game image entirely). The Raise-a-server
wizard generates this file; it is reproduced here as the reference shape:
...
Dashboard wiring for a supervised server: unlike Palworld's stack there is
no REST or RCON port to reach, because the game has neither. Everything the
dashboard reads — liveness, the player list, logs, saves, config — arrives
over the agent's own API, so the server row needs only the agent URL and
token. The game port is display metadata: what a player types to join.
...
4. **Remove the companion agent service** from wherever it lives, and
   ...
   the control plane's app: the agent is the game server now, and the
   separate-stacks rule applies to it.
5. **Update the server row** if ports or the agent URL changed (a
   standalone stack publishes the agent port instead of sharing the control plane's
   network).
...
- **Palcon restarts never touch game servers**, in either mode. The agent
  is not a child of the control plane and holds no live connection; the relationship is
  request/response. Long-running verbs are **jobs**: `POST` starts the work
  and returns immediately, the base polls status. A restart mid-update
...
- **Stopping is asked before it is imposed, in both modes** — but the two
  ...
  *process group* (RSDragonwildsServer.sh is a wrapper; signalling only the script
  ...
  143)`). So the base passes `?graceful=` when its in-game shutdown was
...
- **Supervisor mode couples the game to the *agent image* only**: updating
  ...
  so it updates rarely; the base orchestrates agent updates like scheduled
...
joined by a shared external network. `docker compose down` on the control plane
...
never service blocks to paste into the control plane's own stack.

Companion-mode example (game server stack):

```yaml
# dragonwilds-main/docker-compose.yml — one stack per game server
services:
  dragonwilds:
  ...
  palagent:
    ...
    volumes:
      ...
      # the same stance the base's own save mounts always had. Remove this
...
Per-agent bearer token, generated by the admin (the UI suggests one),
...
- Palcon-side safety that the agent can't provide in companion mode lives
  in the base: SteamCMD update is refused while the container reports running.
...
spinning up a new Dragonwilds server from the dashboard — lands in two steps:
...
- **Phase 5 (optional), one-click — SHIPPED 2026-07**: a provisioner-mode
  ...
  nothing else. When the base has `PROVISIONER_URL`/`PROVISIONER_TOKEN`
  ...
  same way; the base registers no server row and returns the error. A
  ...
  showing as a server in the rail that the dashboard could never reach, since
  ...
  run-as/image-tag defaults; the base adds a free-port proposal computed
  ...
  the dashboard still defaults to dropping just the row — destroying the
  ...
  **A container holding the docker socket as root IS root on the host** —
  ...
  1. **Its API.** Bounded by design: locked verbs, template in code,
     ...
     out Dragonwilds-supervisor containers from this repo's image — fill a
     ...
     reach it over the compose network by service name, so only the base
  ...
  **keep it stopped and start it only while provisioning** — the base
...
1. **Agent skeleton** — SHIPPED 2026-07 (API v1): token auth, `/health`,
   ...
   Wildskeeper UI. Field-validated on a real TrueNAS deployment.
...
- A second published image to build/version (multi-arch), with `/health`
  reporting `apiVersion` so an old agent keeps working with a new control plane.
