...
The checklist now has a second worked example in-tree:
`internal/games/dragonwilds/` (background in `docs/dragonwilds-recon.md`).
It added two shared-layer pieces later games inherit: a client method can
return `*game.UnsupportedError` and the API answers 501 ("this game can't")
instead of 502 ("the server didn't answer"), and the frontend picks page
components per game via `web/src/components/GamePage.tsx` — routes still
never change per game, only what renders at them.
