...
All files live in `web/palcon/src/data/`:
...
| `palCombat.json` | id → [hp, shotAttack, defense, 3 friendship rates] | palworld-save-pal data, calibrated (see `web/palcon/src/lib/stats.ts` header) |
...
| `partnerSkills.json` | species key → partner skill name, cleaned description, and machine-usable tags (mount kind, player attack-element change, element-scoped party buff, species pair-bond, ranch, base-wide work buff) — the Team builder's reasoning data | **generated by `data/vendor_theorycraft.py`** from PalworldSaveTools `characters.json`; tags are parsed from the English descriptions at vendor time — see the note below |
| `palWork.json` | species key → nonzero work suitability levels, food amount, nocturnal flag, ride/transport speeds | same generator, from palworld-save-pal `pals.json` |
...
Not in `web/palcon/src/data/`, but vendored the same way: **`STATUS_NAMES` in
`games/palworld/palsave/extract_pals.py`** maps the Japanese stat names every save
...
(`web/palcon/src/pages/ServerInventory.tsx`) and mirrors the same source, so every
player's build panel reads in one order.

Also not in `web/palcon/src/data/`: the roster tables in **`web/palcon/src/lib/achievements.ts`**.
...
`web/palcon/src/lib/achievements.ts` for the working. Still inference rather than a
...
Pal names and elements are baked into the JSON so `web/palcon/src/lib/fieldBosses.ts`
...
### partnerSkills.json / palWork.json — the one generator that's checked in

`data/vendor_theorycraft.py` regenerates both files; run it with no arguments
to fetch the upstreams' main branches, or pass local copies. Three things it
does beyond copying, all worth knowing on refresh:

- **Tags are parsed from prose.** Mount kind, attack-element change, element
  buffs and the two species pair-bonds (Melpaca → Kingpaca, Beegarde →
  Elizabee) come from regexes over the English skill descriptions. A game
  patch that rewords a description silently drops its tag — the pal keeps its
  name and description, the Team builder just stops crediting the effect. The
  generator prints tag counts; compare them against the previous run.
- **Numbers are deliberately elided.** Descriptions carry
  `{Passive1_EffectValue1}`-style placeholders because the real figures scale
  with partner-skill rank, which no static catalog can know. The cleaner
  patches the grammar around the elision; the team scorer counts effects by
  kind, never by size.
- **The upstreams cross-check each other.** psp's work suitability against
  PalworldSaveTools', and psp's elements against our `palDex.json` — zero
  disagreements on either at vendor time (2026-08). Disagreements print;
  more than a handful means the two repos straddle a game version, so wait.

The team scorer's weights in `web/palcon/src/lib/team.ts` (`W`) are ours, not game
...
it from Rank at runtime, and `condenseAdds` in `web/palcon/src/lib/crew.ts`
...
- Weaknesses are computed by `elementCounters` in `web/palcon/src/lib/paldex.ts` from
...
`EFFIGY_KINDS` in `web/palcon/src/lib/achievements.ts` is the other hand-maintained
...
The element glyphs in `web/palcon/src/components/ElementIcon.tsx` are **not** vendored
...
`web/palcon/src/lib/structures.ts`, which is the easy half: nobody finds a treasure
...
- **Soul rank cap** — 20 (+3% each) in `web/palcon/src/lib/stats.ts` (`soulMult`)
...
- **Uncatchable deck entries** — `UNCATCHABLE_DECK_LABELS` in
  `web/palcon/src/lib/paldex.ts`: Paldeck numbers the game lists but never lets a
...
- **Passive inheritance rates** — `web/palcon/src/lib/inheritance.ts`: the
