// Package palapi is Palworld's contributed console routes — the
// save-derived views (pals, guilds, inventory, storage, achievements)
// that made palcon palcon, mounted through core's game-routes seam and
// fed by the palsave reader the console main constructs.
package palapi
...
// Mount builds the contributed per-server routes. Feature gating happens
// inside each handler (api.RequireFeature), exactly as it always did, so
// the visibility switches keep their meaning.
func Mount(s *api.Server, reader *palsave.Reader) func(chi.Router) {
	h := &handlers{s: s, reader: reader}
	return func(r chi.Router) {
		r.Get("/pals", h.handleServerPals)
		r.Get("/guilds", h.handleServerGuilds)
		r.Get("/inventory", h.handleServerInventory)
		r.Get("/storage", h.handleServerStorage)
		r.Get("/achievements", h.handleServerAchievements)
	}
}
...
func (h *handlers) readSaveForRequest(w http.ResponseWriter, r *http.Request, features ...string) (*palsave.Result, *store.Server, bool) {
	srv, ok := h.s.LoadServer(w, r)
	if !ok {
		return nil, nil, false
	}
	if !api.RequireFeature(w, r, srv, features...) {
		return nil, nil, false
	}
	savePath, err := h.s.FilesHandle().SavePath(r.Context(), srv)
	if errors.Is(err, agentfiles.ErrNotConfigured) {
		api.WriteError(w, http.StatusBadRequest, "no save path configured")
		return nil, nil, false
	}
	if err != nil {
		h.s.LoggerHandle().Error("save sync from agent failed", "server", srv.ID, "error", err)
	...
	result, err := h.reader.ReadServeStale(r.Context(), savePath)
	if errors.Is(err, palsave.ErrNotConfigured) {
		api.WriteError(w, http.StatusBadRequest, "no save path configured")
		return nil, nil, false
	}
	if err != nil {
		h.s.LoggerHandle().Error("save extraction failed", "server", srv.ID, "error", err)
		api.WriteError(w, http.StatusBadGateway, err.Error())
		return nil, nil, false
	}
...
func (h *handlers) handleServerPals(w http.ResponseWriter, r *http.Request) {
	// One payload, three views: Player pals, Paldex and Calculators all read
	// it, so it answers while any of them is on.
	result, srv, ok := h.readSaveForRequest(w, r, store.FeaturePals, store.FeaturePaldex, store.FeatureCalculators)
	if !ok {
		return
	}
	hidden, err := h.s.HiddenPlayers(r, srv.ID)
	if err != nil {
		api.WriteError(w, http.StatusInternalServerError, err.Error())
		return
	}
	api.WriteJSON(w, http.StatusOK, map[string]any{
		"players":     toPalsPlayers(visiblePlayers(result.Players, hidden, store.StreamPals), h.lastSeen(r, srv)),
...
func (h *handlers) lastSeen(r *http.Request, srv *store.Server) lastSeenIndex {
	idx := lastSeenIndex{canon: srv.CanonicalUID}
	seen, err := h.s.StoreHandle().LastSeen(r.Context(), srv.ID)
	if err != nil {
		h.s.LoggerHandle().Error("reading last-seen history", "server", srv.ID, "error", err)
		return idx
	}
	idx.at = seen
	return idx
...
func (h *handlers) handleServerInventory(w http.ResponseWriter, r *http.Request) {
	result, srv, ok := h.readSaveForRequest(w, r, store.FeatureInventory)
	if !ok {
		return
	}
	hidden, err := h.s.HiddenPlayers(r, srv.ID)
...
func (h *handlers) handleServerAchievements(w http.ResponseWriter, r *http.Request) {
	result, srv, ok := h.readSaveForRequest(w, r, store.FeatureAchievements)
	if !ok {
		return
	}
	hidden, err := h.s.HiddenPlayers(r, srv.ID)
...
func (h *handlers) handleServerStorage(w http.ResponseWriter, r *http.Request) {
	...
	result, srv, ok := h.readSaveForRequest(w, r, store.FeatureStorage)
	if !ok {
		return
	}
...
// either view is on.
func (h *handlers) handleServerGuilds(w http.ResponseWriter, r *http.Request) {
	result, srv, ok := h.readSaveForRequest(w, r, store.FeatureGuilds, store.FeatureMap)
	if !ok {
		return
	}
	hidden, err := h.s.HiddenPlayers(r, srv.ID)
	...
	api.WriteJSON(w, http.StatusOK, map[string]any{
		"guilds":      result.Guilds,
		"players":     toPalsPlayers(players, h.lastSeen(r, srv)),
		"parsedAt":    result.ParsedAt,
		"saveModTime": result.SaveModTime,
	})
}
