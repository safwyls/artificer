...
func New(conn game.Conn) game.Client {
	...
	base, err := agentctl.New(conn.AgentURL, conn.AgentToken)
	if err != nil {
		return &Client{agentErr: fmt.Errorf("agent: %w", err)}
	}
	return &Client{agent: dwbridge.Wrap(base), tracker: trackerFor(conn.AgentURL)}
}
...
func (c *Client) bridgeReady(ctx context.Context, op string) error {
	...
	bridge := dwbridge.StatusFromHealth(h)
	...
	if !containsString(bridge.Commands, op) {
