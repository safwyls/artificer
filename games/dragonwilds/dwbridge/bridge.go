...
// Package dwbridge is both halves of the dwbridge command channel
...
// DirName is the shared directory under the install root. The launch
// profile points the mod's DWBRIDGE_DIR at the same path.
const DirName = "dwbridge"
...
// ErrUnavailable means no live mod is behind the bridge. It maps to a
...
var ErrUnavailable = errors.New("dwbridge mod is not responding")
...
func New(installDir string) *Bridge {
	return &Bridge{
...
// fails fast with ErrUnavailable instead of hanging to the timeout.
func (b *Bridge) available(command string) error {
	...
	if age > heartbeatFresh {
		return fmt.Errorf("%w: last heartbeat %ds ago", ErrUnavailable, int(age.Seconds()))
	}
...
func (b *Bridge) Command(ctx context.Context, command string, args map[string]string) (json.RawMessage, error) {
	...
	for {
		...
		case <-ctx.Done():
			os.Remove(reqPath)
			return nil, fmt.Errorf("%w: %s timed out", ErrUnavailable, command)
		case <-ticker.C:
		}
	}
}
