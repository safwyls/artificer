        ...
        return (
          <label
            ...
            className={cn(
              ...
              checked ? "border-wk-ember/30 bg-wk-ember/5" : "border-wk-edge hover:bg-wk-ink/[0.03]",
              disabled && "cursor-not-allowed opacity-50",
            )}
          >
            <input
              type="checkbox"
              className="mt-0.5 h-4 w-4 accent-wk-ember"
              checked={checked}
              disabled={disabled}
              onChange={(e) => onChange(e.target.checked ? [...value, p] : value.filter((x) => x !== p))}
            />
            <span className="min-w-0">
              <span className="block text-sm font-semibold text-wk-parchment">{PERMISSION_LABELS[p].label}</span>
              <span className="block text-xs text-wk-parchment/50">{PERMISSION_LABELS[p].help}</span>
            </span>
          </label>
        );
      })}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <p className="-mt-2 text-xs text-wk-parchment/50">
          ...
          <div>
            ...
            {isAdmin && <p className="mt-2 text-xs text-wk-parchment/50">Administrators already have every permission.</p>}
          </div>
    ...
    <div>
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-wk-edge bg-wk-bg px-4 py-5 lg:px-8 lg:py-6">
        <div>
          ...
          <p className="mt-0.5 text-sm text-wk-parchment/50">Who can sign in, and what they're allowed to do</p>
        </div>
      ...
      <div className="space-y-3 p-4 lg:p-8">
        ...
        {usersQuery.data?.map((u) => {
          ...
          return (
            <section
              key={u.id}
              className="flex flex-wrap items-center gap-3 rounded-2xl border border-wk-edge bg-wk-panel p-4"
            >
              <span
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate font-display text-base font-bold">
                  {u.username}
                  {u.username === me && <span className="ml-2 text-xs font-normal text-wk-parchment/40">you</span>}
                  {u.disabled && <span className="ml-2 text-xs font-normal text-destructive">disabled</span>}
                </p>
                <p className="font-mono text-xs text-wk-parchment/45">
                  {u.role === "admin"
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  size="icon"
                  title="Remove user"
                  className="text-wk-parchment/40 hover:text-destructive"
