  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[85vh] w-[calc(100vw-2rem)] max-w-4xl flex-col">
              ...
              <span className="flex items-center gap-1.5 font-mono text-xs font-normal text-wk-parchment/45">
                ...
                <span
                  className={cn(
                    ...
                    running ? "animate-pulse bg-wk-ok" : job.state === "done" ? "bg-wk-ok" : "bg-wk-ember",
                  )}
        ...
        {job?.error && (
          <p className="rounded-lg bg-wk-ember/10 px-3 py-2 font-mono text-xs text-wk-ember">{job.error}</p>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-wk-parchment/40">Last {lines.length} lines from the agent</span>
        </div>
