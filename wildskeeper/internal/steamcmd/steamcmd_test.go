...
func TestUpdateArgs(t *testing.T) {
	got := strings.Join(UpdateArgs("/palworld", 4019830, true), " ")
	want := "+force_install_dir /palworld +login anonymous +app_update 4019830 validate +quit"
	if got != want {
		t.Errorf("with validate:\n got %q\nwant %q", got, want)
	}
	for _, arg := range UpdateArgs("/palworld", 4019830, false) {
