...
// Dragonwilds game server (or supervises it directly), holding the
// install volume and SteamCMD, and exposes a narrow authenticated API
...
func main() {
	...
	a, err := agent.New(agent.Config{
		...
		Game: dwagent.Game(dwagent.LaunchConfig{
			Profile:      envOr("WKAGENT_LAUNCH_PROFILE", dwagent.ProfileNative),
	...
	addr := envOr("WKAGENT_ADDR", ":8811")
	logger.Info("wkagent listening", "addr", addr, "version", version, "apiVersion", agent.APIVersion)
