...
func main() {
	...
	a, err := agent.New(agent.Config{
		...
		// The game half of the agent: Enshrouded's spec, with the wine
		// tuning from the environment. (FLAMEAGENT_LAUNCH_PROFILE is gone
		// with the chooser — one build exists; a custom GAME_CMD is still
		// honoured verbatim.)
	...
	addr := envOr("FLAMEAGENT_ADDR", ":8811")
	logger.Info("flameagent listening", "addr", addr, "version", version, "apiVersion", agent.APIVersion)
