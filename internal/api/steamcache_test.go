...
func fakeInstallDir(t *testing.T) string {
	...
	for _, p := range []string{
		filepath.Join("steamapps", "downloading", "4019830"),
	...
	for _, f := range []string{
		filepath.Join("steamapps", "appmanifest_4019830.acf"),
		filepath.Join("steamapps", "downloading", "4019830", "chunk"),
...
func TestClearSteamCache(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(install, "RSDragonwildsServer.sh")); err != nil {
		t.Errorf("RSDragonwildsServer.sh should survive: %v", err)
	}
