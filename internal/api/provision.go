...
type provisionRequest struct {
	...
	// ImageTag selects the wkagent channel; default latest.
...
func (s *Server) handleProvisionServer(w http.ResponseWriter, r *http.Request) {
	...
	if req.GamePort == 0 {
		req.GamePort = wkagent.DefaultGamePort
	}
	if req.AgentPort == 0 {
		req.AgentPort = 8811
	}
...
`, req.RunAs)
	...
	identityEnv := fmt.Sprintf("      WKAGENT_SERVER_NAME: %q\n", req.ServerName)
	if req.WorldName != "" {
		identityEnv += fmt.Sprintf("      WKAGENT_WORLD_NAME: %q\n", req.WorldName)
	}

	stack := fmt.Sprintf(`# %s — RuneScape: Dragonwilds server supervised by wkagent,
...
`, req.Name, req.ImageTag, userLine, token, req.AdminPassword, req.OwnerID, identityEnv,
		req.GamePort, wkagent.DefaultGamePort,
		req.GamePort+1, wkagent.DefaultGamePort+1,
	...
	container := ""
	if s.Provisioner != nil {
		result, err := s.Provisioner.Provision(r.Context(), wkagent.ProvisionRequest{
			Slug:          slug,
...
func conflictMessage(slug string) string {
	return fmt.Sprintf("a container named wkagent-%s already exists on the host — "+
...
func proposePorts(servers []*store.Server, containerPorts []int) map[string]int {
	...
	for offset := 0; offset < 1000; offset++ {
		game, agent := wkagent.DefaultGamePort+(offset*2), 8811+offset
		if !used[game] && !used[game+1] && !used[agent] {
			return map[string]int{"game": game, "agent": agent}
		}
	}
	return map[string]int{"game": wkagent.DefaultGamePort, "agent": 8811}
}
...
// handleAdoptServer re-registers a discovered wkagent container as a
...
func (s *Server) handleAdoptServer(w http.ResponseWriter, r *http.Request) {
	...
	if host == "" {
		writeError(w, http.StatusBadRequest,
			"could not infer the host address — set WKAGENT_PUBLIC_HOST on the provisioner or pass one")
		return
	}

	name := adopted.ServerName
	if name == "" {
		name = strings.ReplaceAll(strings.TrimPrefix(adopted.Name, "wkagent-"), "-", " ")
	}
	srv := &store.Server{
		Name: name, Host: host,
