...
`
	if err := os.WriteFile(filepath.Join(install, "PalServer.sh"), []byte(game), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestPowerViaSupervisorAgent(t *testing.T) {
	...
	if state.Running || !strings.Contains(state.Name, "palagent") {
		t.Fatalf("initial state = %+v, want stopped palagent-managed", state)
	}
