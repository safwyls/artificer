...
func newDockerFake(t *testing.T) (*dockerFake, *dockerctl.Client) {
	t.Helper()
	f := &dockerFake{state: `{"Name":"/palagent-main","State":{"Status":"running","Running":true,"StartedAt":"2026-08-04T10:00:00Z"}}`}
...
func TestContainerStatus(t *testing.T) {
	...
	base := "/api/servers/" + itoa(addContainerServer(t, app, "palagent-main"))
	...
	if state.Name != "palagent-main" || !state.Running || state.Status != "running" {
...
func TestContainerActions(t *testing.T) {
	fake, docker := newDockerFake(t)
	app, admin := newTestAppWithDocker(t, docker)
	id := addContainerServer(t, app, "palagent-main")
