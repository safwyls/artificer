...
// can't brick the server's boot, keep a one-level .dwcon.bak, swap the file
...
// previous contents in a sibling .dwcon.bak for one-level undo.
func atomicWrite(file string, data []byte) error {
	...
	if info, err := os.Stat(file); err == nil {
		...
		if orig, err := os.ReadFile(file); err == nil {
			_ = os.WriteFile(file+".dwcon.bak", orig, mode)
		}
	}
