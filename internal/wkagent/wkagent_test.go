...
func TestAgentUpdateJob(t *testing.T) {
	srv, _ := newTestAgent(t, `echo "Update state (0x61) downloading"; echo "Success! App '4019830' fully installed."`)
...
func TestAgentUpdateFailures(t *testing.T) {
	...
	srv, _ := newTestAgent(t, `echo "Error! App '4019830' state is 0x602 after update job."; exit 0`)
...
if [ ! -f "$sentinel" ]; then
  touch "$sentinel"
  echo "ERROR! Failed to install app '4019830' (Missing configuration)"
  exit 0
fi
...
func TestAgentUpdatePersistentErrorFailsOnce(t *testing.T) {
	srv, _ := newTestAgent(t, `echo "ERROR! Failed to install app '4019830' (Missing configuration)"; exit 0`)
	_, m := do(t, srv, "POST", "/v1/steam/update", testToken, nil)
	job := waitForJob(t, srv, m["job"].(map[string]any)["id"].(string))
