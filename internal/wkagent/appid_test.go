...
// never uses (see wkagent.DefaultAppID). This test is where the two are
// held together — a test-only import, so it costs the shipped binary
// nothing.
func TestDefaultAppIDMatchesDragonwilds(t *testing.T) {
	if wkagent.DefaultAppID != dragonwilds.AppID {
		t.Errorf("wkagent.DefaultAppID = %d, dragonwilds.AppID = %d — the agent would update the wrong app",
			wkagent.DefaultAppID, dragonwilds.AppID)
	}
}
