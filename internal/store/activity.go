...
// After a clean run that is exactly who is online; after flametender was killed
...
// stayed. Flametender's own observations are the only record of when a player
// actually went away.
//
...
// the last moment flametender can honestly claim to have watched them, which after
...
func (s *Store) LastSeen(ctx context.Context, serverID int64) (map[string]time.Time, error) {
	...
	for rows.Next() {
		...
		if event == "join" {
			...
			// older than the join means flametender has not looked since.
			if at.Before(watch) {
				at = watch
			}
		}
...
// list, bounding how much of an open session flametender may later claim to have
...
// AuditEntry is one management action taken through Flametender.
