...
func seedWorld(t *testing.T, install string) {
	t.Helper()
	world := filepath.Join(install, "Pal", "Saved", "SaveGames", "0", "WORLDA")
	for _, d := range []string{filepath.Join(world, "Players"), filepath.Join(world, "backup"), filepath.Join(install, "Pal", "Saved", "Config", "LinuxServer")} {
	...
	files := map[string]string{
		...
		filepath.Join(install, "Pal", "Saved", "Config", "LinuxServer", "PalWorldSettings.ini"): "[/Script/Pal.PalGameWorldSettings]\nOptionSettings=(ExpRate=1.0)\n",
	}
...
func TestAgentConfigRoundTrip(t *testing.T) {
	srv, install := newTestAgent(t, "exit 0")
	seedWorld(t, install)
	iniPath := filepath.Join(install, "Pal", "Saved", "Config", "LinuxServer", "PalWorldSettings.ini")

	resp, body := rawGet(t, srv.URL, "/v1/files/config", "")
	if resp.StatusCode != http.StatusOK || !bytes.Contains(body, []byte("ExpRate=1.0")) {
		t.Fatalf("get config: %d %q", resp.StatusCode, body)
	}

	newIni := "[/Script/Pal.PalGameWorldSettings]\nOptionSettings=(ExpRate=2.0)\n"
