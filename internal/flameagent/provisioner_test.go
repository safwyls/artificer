...
func (f *fakeDockerAPI) handler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/containers/json":
			...
			w.Write([]byte(`[
			  {"Id":"c1","Names":["/flameagent-main"],"Image":"ghcr.io/safwyls/flameagent:beta","State":"running",
			  ...
			  {"Id":"c2","Names":["/wkprovisioner"],"Image":"ghcr.io/safwyls/flameagent:beta","State":"running","Ports":[]},
			  {"Id":"c3","Names":["/nginx"],"Image":"nginx:latest","State":"running","Ports":[]}
			]`))
		case r.URL.Path == "/containers/c1/json":
			w.Write([]byte(`{"Config":{"Env":["FLAMEAGENT_MODE=supervisor","FLAMEAGENT_TOKEN=secret-must-not-leak","FLAMEAGENT_ADMIN_PASSWORD=recovered-pw","FLAMEAGENT_SERVER_NAME=Main World"]}}`))
		case r.URL.Path == "/containers/c2/json":
			w.Write([]byte(`{"Config":{"Env":["FLAMEAGENT_MODE=provisioner"]}}`))
		default:
...
func TestProvisionerCreatesServer(t *testing.T) {
	...
	if m["container"] != "flameagent-palhalla-2" || m["dataDir"] != filepath.Join(dataRoot, "palhalla-2") {
	...
	// pull → create → start, template locked to the flameagent image.
	...
	if fake.create["Image"] != "ghcr.io/safwyls/flameagent:beta" || fake.create["User"] != "568:568" {
		t.Errorf("create = image %v user %v", fake.create["Image"], fake.create["User"])
	}
	env := strings.Join(toStrings(fake.create["Env"].([]any)), " ")
	for _, want := range []string{"FLAMEAGENT_MODE=supervisor", "FLAMEAGENT_TOKEN=new-agent-token", "FLAMEAGENT_ADMIN_PASSWORD=pw12345", "FLAMEAGENT_SERVER_NAME=Palhalla II", "HOME=/tmp"} {
...
// the image pull, and with 409 — the status flamekeeper reads as "nothing was
...
func TestProvisionerRefusesNameInUse(t *testing.T) {
	...
	if msg, _ := m["error"].(string); !strings.Contains(msg, "flameagent-main") {
...
func TestProvisionerHealthDefaultsAndDiscover(t *testing.T) {
	...
	if got["name"] != "flameagent-main" || got["mode"] != "supervisor" || got["running"] != true ||
...
// control plane recovers a lost registration. Restricted to flameagent
// containers.
func TestProvisionerAdopt(t *testing.T) {
	srv, _, _ := newProvisioner(t)

	resp, m := do(t, srv, "POST", "/v1/adopt", testToken, map[string]string{"container": "flameagent-main"})
	if resp.StatusCode != http.StatusOK {
	...
	// Not-a-flameagent and unknown containers refuse.
...
func TestProvisionerDestroy(t *testing.T) {
	...
	resp, m := do(t, srv, "POST", "/v1/destroy", testToken, map[string]string{"container": "flameagent-main"})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("destroy: %d %v", resp.StatusCode, m)
	}
	if m["container"] != "flameagent-main" || m["dataDir"] != filepath.Join(dataRoot, "main") {
...
// only containers this provisioner created can be destroyed. A flameagent
...
func TestProvisionSpecPlacesAnotherGamesServer(t *testing.T) {
	...
	if labels["flamekeeper.provisioned"] != "true" || labels["flamekeeper.slug"] != "palhalla" {
...
func TestProvisionSpecRefusesSlugTraversal(t *testing.T) {
	...
	for _, slug := range []string{"../etc", "a/b", "/abs", ".."} {
		resp, _ := do(t, srv, "POST", "/v1/provision/spec", testToken, map[string]any{
			"name": "x", "slug": slug, "image": "ghcr.io/safwyls/flameagent:latest",
		})
		if resp.StatusCode != http.StatusBadRequest {
...
func TestProvisionSpecCannotForgeOwnershipLabels(t *testing.T) {
	...
	do(t, srv, "POST", "/v1/provision/spec", testToken, map[string]any{
		"name": "flameagent-x", "slug": "x", "image": "ghcr.io/safwyls/flameagent:latest",
		"labels": map[string]string{"flamekeeper.slug": "someone-elses", "mine": "ok"},
	})
	labels, _ := fake.create["Labels"].(map[string]any)
	if labels["flamekeeper.slug"] != "x" {
