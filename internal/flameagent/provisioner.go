...
// template lives here in code: a compromised flamekeeper (or leaked provisioner
...
type ProvisionRequest struct {
	...
	// (FLAMEAGENT_ADMIN_PASSWORD), enforced into DedicatedServer.ini.
	...
	// flameagent/uid 1000 — never root, which the game refuses to boot as).
...
func (a *Agent) handleProvision(w http.ResponseWriter, r *http.Request) {
	...
	spec := ProvisionSpec{
		Name:  name,
		Slug:  req.Slug,
		Image: "ghcr.io/safwyls/flameagent:" + req.ImageTag,
		User:  req.RunAs,
		Env:   env,
...
type ProvisionDefaults struct {
	...
	// PublicHost is the address flamekeeper (and players) reach this host on —
	// FLAMEAGENT_PUBLIC_HOST. Inside containers "localhost" means the
...
// handleDiscover lists flameagent-shaped containers on the host so the add
...
func (a *Agent) handleDiscover(w http.ResponseWriter, r *http.Request) {
	...
	for _, c := range containers {
		if !strings.Contains(c.Image, "flameagent") && c.Labels["flamekeeper.provisioned"] != "true" {
			continue
		}
		mode := ""
		if env, err := a.docker.InspectEnv(r.Context(), c.ID); err == nil {
			for _, e := range env {
				...
				if v, ok := strings.CutPrefix(e, "FLAMEAGENT_MODE="); ok {
					mode = v
				}
			}
		}
		...
		if mode == "" {
			mode = "companion" // flameagent's default mode
		}
...
// AdoptResult carries everything flamekeeper needs to re-register an existing
// flameagent container — including its secrets. Deliberately: the
...
// flameagent containers — never arbitrary ones.
...
func (a *Agent) handleAdopt(w http.ResponseWriter, r *http.Request) {
	...
	for _, c := range containers {
		...
		if !strings.Contains(c.Image, "flameagent") && c.Labels["flamekeeper.provisioned"] != "true" {
			writeError(w, http.StatusBadRequest, "not a flameagent container")
			return
		}
		...
		for _, e := range env {
			...
			if v, ok := strings.CutPrefix(e, "FLAMEAGENT_TOKEN="); ok {
				res.Token = v
			}
			if v, ok := strings.CutPrefix(e, "FLAMEAGENT_ADMIN_PASSWORD="); ok {
				res.AdminPassword = v
			}
			if v, ok := strings.CutPrefix(e, "FLAMEAGENT_SERVER_NAME="); ok {
				res.ServerName = v
			}
		}
...
//
// The label gate is the whole security argument. `flamekeeper.provisioned=true`
...
// discover/adopt, which also match on the flameagent image name: a
// hand-deployed flameagent (a TrueNAS app, a pasted stack) is something the
...
func (a *Agent) handleDestroy(w http.ResponseWriter, r *http.Request) {
	...
	for _, c := range containers {
		...
		if c.Labels["flamekeeper.provisioned"] != "true" {
		...
		dataDir := ""
		if slug := c.Labels["flamekeeper.slug"]; slug != "" {
...
type RecreateRequest struct {
	...
	Container string `json:"container"`
	// ImageTag is the flameagent channel to move to, e.g. "latest-wine".
	ImageTag string `json:"imageTag"`
}
...
func (a *Agent) handleRecreate(w http.ResponseWriter, r *http.Request) {
	...
	if found.Labels["flamekeeper.provisioned"] != "true" {
		writeError(w, http.StatusBadRequest,
...
func (a *Agent) place(ctx context.Context, spec ProvisionSpec) (string, string, error) {
	...
	labels["flamekeeper.provisioned"] = "true"
	labels["flamekeeper.slug"] = spec.Slug
