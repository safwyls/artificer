...
// finish before the supervisor escalates. Flamekeeper asks the game to shut
...
func (s *supervisor) seedConfig(ini string) {
	if s.ownerID == "" {
		s.logger.Warn("no owner id configured: the game will write its own DedicatedServer.ini and refuse to start until OwnerId is filled in",
			"path", ini, "fix", "set FLAMEAGENT_OWNER_ID (in-game: Settings, bottom-left 'My Player ID')")
		return
	}
	...
	b.WriteString("; Seeded by flameagent on first install. The game owns this file from\n")
