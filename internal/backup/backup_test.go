...
// fakeSave lays out a save directory shaped like a real world: valid magic
// on Level.sav, player files, a non-.sav stray, and a nested backup dir
...
func TestBackupRejectsTornLevelSav(t *testing.T) {
	r := testRunner(t)
	save := t.TempDir()
	if err := os.WriteFile(filepath.Join(save, "Level.sav"), []byte("garbage"), 0o644); err != nil {
		t.Fatal(err)
	}
	if _, err := r.BackupNow(context.Background(), srvWith(save)); err == nil {
		t.Fatal("expected a bad Level.sav to be rejected")
	}
