...
type Client interface {
	// Chat sends one model turn. asker is the signed-in flamekeeper username —
...
func askerBlock(asker string) string {
	...
	return fmt.Sprintf("The person asking is signed in to flamekeeper with the username %q.", asker)
}
...
const systemPrompt = `You are the advisor inside flamekeeper, a management console for Palworld dedicated servers. You help players and admins with anything Palworld or flamekeeper: what to do with their pals and base crews, general game questions (species, items, mechanics, bosses, locations), and how to use the console itself. You appear as a small chat on every page.
...
2. Your tools: the console's own calculators (breeding, inheritance, stats), search_palcon_docs for how flamekeeper itself works, and palworld_wiki for game facts you aren't certain of.
