...
// ConfigPath returns a local directory holding PalWorldSettings.ini for
...
func (s *Syncer) ConfigPath(ctx context.Context, srv *store.Server) (path string, viaAgent bool, err error) {
	...
	if err := os.WriteFile(filepath.Join(dir, "PalWorldSettings.ini"), data, 0o600); err != nil {
		return "", false, err
	}
	return dir, true, nil
}
...
func (s *Syncer) PushConfig(ctx context.Context, srv *store.Server, dir string) error {
	...
	data, err := os.ReadFile(filepath.Join(dir, "PalWorldSettings.ini"))
	if err != nil {
		return err
	}
