...
Flametender can take the identity Cloudflare Access has already
...
3. `internal/cfaccess` verifies the token: signature against the team's
   published keys, issuer, **audience**, expiry, and that it is an
   application token rather than the team-wide session one.
4. The email becomes a Flametender account — found, or created on the
...
hostname your tunnel serves Flametender on, and give it a policy (an
...
arrives with a `common_name` and no email. Flametender has no machine
...
So Flametender maps permissions in its own Users page, and uses the
...
- Implementation: `internal/cfaccess` (verification, with tests covering
  cross-application tokens, algorithm confusion, key rotation and expiry)
  and `internal/api/auth.go` (`handleCloudflareLogin`, account handling).
