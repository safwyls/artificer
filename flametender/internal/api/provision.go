...
// Provisioning ("new server from the dashboard"): flametender deliberately
...
func (s *Server) handleProvisionServer(w http.ResponseWriter, r *http.Request) {
	...
	case req.Name == "":
		...
		return
	case req.Host == "":
		writeError(w, http.StatusBadRequest, "host is required — the address Flametender will reach the server on")
		return
...
# Flametender. Deploy as its own stack (TrueNAS custom app / docker
...
// the agent's port are both free of anything flametender tracks or the
