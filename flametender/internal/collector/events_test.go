...
// The bug this guards: flametender restarting while someone is online used to
...
func TestRestartClosesSessionsAtLastObservation(t *testing.T) {
	...
	// Run 1: Kyoshi is on when flametender starts watching, and still on when
	// flametender is killed without a chance to clean up.
	...
	// The session must be bounded by what flametender watched, not open-ended.
...
// observation stopped and a new one opened where it resumed, so flametender's
...
// The graceful path: stopping flametender ends the sessions it knows about, so a
